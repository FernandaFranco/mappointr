...
class ApplicationSystemTestCase < ActionDispatch::SystemTestCase
  ...
  # testes de sala) carregam session[:user_id] de um método de teste pro
  ...
  # deixou a sessão :host identificada como fernanda, o próximo teste que
  # tentar sign_in_as(outro_user) em :host só teria efeito se de fato
  # substituir esse cookie — sem o reset, um teste poderia silenciosamente
  # herdar o jogador errado do teste anterior. Isso só se manifestava
  # rodando a suíte inteira (nunca um arquivo sozinho com poucos usos de
  # using_session), e o teste que falhava mudava a cada rodada conforme a
  # ordem aleatória do --seed.
  ...
  # Sem login: fixa qual User esta sessão de browser representa via a rota
  # só de teste (ver TestSignInsController) — visit é síncrono (espera o
  # redirect terminar), então não precisa de sincronização extra depois.
  def sign_in_as(user)
